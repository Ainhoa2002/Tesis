"""
Role: Magnet calculations and helper routines.

Brief: Contains functions to compute magnet masses, material breakdowns, and
other magnet-specific calculations used in the LCI magnet pipeline.
"""

import math
import logging

rho_cu = 8960          # kg/m^3
rho_fe = 4800
L_ind1 = 5.9             # m
d_ind1 = 2e-3            # m
L_t1 = 2*3.9 +0.4           # m
d_t1 = 0.8e-3            # m
L_t2 = 2*3*0.72+0.12             # m
d_t2 = 1.5e-3            # m



m_cu1 = rho_cu * L_ind1 * math.pi * (d_ind1/2)**2
m_cut1 = rho_cu * L_t1 * math.pi * (d_t1/2)**2
m_cut2 = rho_cu * L_t2 * math.pi * (d_t2/2)**2

m_fe1 = 0.338847
m_ny1 = 0.0012
m_ptfe1= 0.0016
m_plas1=0.0338

m_total1 = m_cu1 + m_fe1 + m_ny1 + m_ptfe1
per_cu1 = m_cu1 / m_total1 * 100
per_fe1 = m_fe1 / m_total1 * 100
per_ny1 = m_ny1 / m_total1 * 100
per_ptfe1 = m_ptfe1 / m_total1 * 100
per_plas1 = m_plas1 / m_total1 * 100
logging.info("%s %s %s %s", L_t1, d_t1, L_t2, d_t2)
logging.info("%s", m_cu1)
logging.info("%s", m_cut1)
logging.info("%s", m_cut2)
logging.info("%s", m_total1)
logging.info("Copper: %.2f%%", per_cu1)
logging.info("Steel: %.2f%%", per_fe1)
logging.info("Nylon: %.2f%%", per_ny1)
logging.info("PTFE: %.2f%%", per_ptfe1)
logging.info("Plastic: %.2f%%", per_plas1)