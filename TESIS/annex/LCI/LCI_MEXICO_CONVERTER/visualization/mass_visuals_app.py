...
def _build_summary_table(view: pd.DataFrame, group_col: str) -> pd.DataFrame:
    ...
    table["Percent"] = table["mass_kg"] / total_mass * 100 if total_mass and total_mass > 0 else 0.0
    formatted = table.copy()
    formatted["mass_kg"] = formatted["mass_kg"].map(lambda x: f"{x:.6f}")
    formatted["Percent"] = formatted["Percent"].map(lambda x: f"{x:.2f}%")
    return formatted
...
def main() -> None:
    ...
    view2b = view.copy()
    view2b["Category_for_treemap"] = view2b["Category"].apply(_clean_category)
    fig2b = px.treemap(
        view2b,
    ...
    st.dataframe(_build_summary_table(view, "Subsystem").reset_index(drop=True), use_container_width=True)

    # Section summary table: total mass and percentage of current view
    st.subheader("Section mass summary")
    st.dataframe(_build_summary_table(view, "Section").reset_index(drop=True), use_container_width=True)
